  ...
  if (!product) {
    return (
      <div className="container-main py-20 text-center">
        ...
        <p className="font-semibold text-zinc-700 dark:text-zinc-300">Mahsulot topilmadi</p>
  ...
  return (
    <div className="container-main py-6 lg:py-10 pb-16">
      ...
      <nav className="mb-6 flex items-center gap-1.5 text-sm text-zinc-400 dark:text-zinc-500">
        <Link href={ROUTES.home}     className="hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">Bosh sahifa</Link>
        <span>/</span>
        <Link href={ROUTES.products} className="hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">Mahsulotlar</Link>
        <span>/</span>
        <span className="truncate text-zinc-600 dark:text-zinc-400 max-w-[200px]">{t(product.title)}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-14">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeImg === i
                      ...
                      : 'opacity-55 hover:opacity-90 hover:ring-1 hover:ring-zinc-300 dark:hover:ring-zinc-600'
                  )}
                >
        ...
        <div className="space-y-5">
          ...
          {/* Title */}
          <h1 className="font-display text-2xl font-bold leading-tight text-zinc-900 dark:text-white sm:text-3xl text-pretty">
            {t(product.title)}
          </h1>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-4">
              <span className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">{tr('qty')}:</span>
              <div className="flex items-center overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-700">
                <button
                  onClick={() => setQty(q => Math.max(1, q - 1))}
                  className="flex h-10 w-10 items-center justify-center text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
                >
                  <Minus className="h-4 w-4" />
                </button>
                <span className="w-10 text-center text-sm font-bold text-zinc-900 dark:text-zinc-100">{qty}</span>
                <button
                  onClick={() => setQty(q => Math.min(product.stockQty, q + 1))}
                  disabled={qty >= product.stockQty}
                  className="flex h-10 w-10 items-center justify-center text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors disabled:opacity-40"
                >
                  <Plus className="h-4 w-4" />
                </button>
              </div>
              <span className="text-sm text-zinc-400 dark:text-zinc-500">{product.stockQty} ta mavjud</span>
            </div>
          ...
          <div className="rounded-2xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 p-4 space-y-3">
            {[
              ...
              <div key={text} className="flex items-center gap-2.5 text-sm text-zinc-600 dark:text-zinc-400">
      ...
      <div className="mt-12 lg:mt-16">
        {/* Tab switcher */}
        <div className="flex border-b border-zinc-200 dark:border-zinc-800 mb-8 gap-1">
          {([
          ...
          <div className="prose prose-stone dark:prose-invert max-w-3xl animate-fade-in">
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4 not-prose">
              {[
                ...
                <div key={label} className="rounded-xl border border-zinc-100 dark:border-zinc-800 p-3.5">
                  <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-1">{label}</p>
                  <p className="font-semibold text-zinc-800 dark:text-zinc-200 text-sm">{value}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'reviews' && (
          <div className="space-y-5 max-w-3xl animate-fade-in">
            {!reviews?.length && (
              <p className="text-zinc-400 dark:text-zinc-500 text-sm">Hali sharhlar yo'q.</p>
            )}
            {reviews?.map(review => (
              <div key={review.id} className="rounded-2xl border border-zinc-100 dark:border-zinc-800 p-5 space-y-2.5">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{review.userName}</p>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-zinc-400 mt-0.5">{timeAgo(review.createdAt)}</p>
                  </div>
                </div>
